...
2. A Next.js UI allowing visualisation of the agent orchestration process and providing a chat interface. The UI calls backend `chatkit`-compatible endpoints and renders a custom customer panel plus an agent activity panel.
...
This command also starts the backend process.
...
This repository includes hands-on Codex workshop examples under [`examples/`](examples):
...
4. [`examples/04-audit-codebase/README.md`](examples/04-audit-codebase/README.md): run the `$audit` skill and use a reusable skill-first review workflow.

For facilitator sequencing and timing, see [`WORKSHOP.md`](WORKSHOP.md).
